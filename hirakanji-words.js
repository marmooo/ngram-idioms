...
if (Deno.args.length < 1) {
  console.log("USAGE: hirakanji-words.js [threshold]");
  Deno.exit(1);
}
...
function isValidLevel(word, level) {
  const chars = word.split("");
  for (let i = 0; i < chars.length; i++) {
    if (level < 10) {
...
async function listupFromWords(mecab, level) {
  ...
  for await (const line of readLines(fileReader)) {
    ...
    let idiom = arr.slice(0, -1).join("");
    const count = parseInt(arr.slice(-1));
    if (
      isValidWord(idiom, level) && isValidLevel(idiom, level) &&
      isBasicExample(idiom)
    ) {
      const parsed = await mecab.parse(idiom);
      if (parsed.length == 1) {
        const pos1 = parsed[0].feature;
        if (pos1 == "名詞") {
          if (parsed[0].featureDetails.includes("固有名詞")) {
            continue;
          }
        } else if (pos1 == "助詞") {
          continue;
        } else if (pos1 == "動詞" || pos1 == "形容詞") {
          idiom = parsed[0].originalForm; // 原形に正規化
        }
        if (idioms[idiom]) {
          idioms[idiom] += count;
        } else {
...
const baseDir = `hirakanji-${thresholdName}`;
Deno.mkdirSync(baseDir, { recursive: true });
for (let level = 1; level <= 10; level++) {
  const idioms = await listupFromWords(mecab, level);
