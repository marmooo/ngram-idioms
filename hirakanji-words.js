...
const MeCab = new require('mecab-async'), mecab = new MeCab();
...
var idioms = {};
readEachLineSync('nwc2010-ngrams/word/over999/1gms/1gm-0000', 'utf8', (line) => {
  var [idiom, count] = line.split(/\s/);
  count = parseInt(count);
  if (isValidWord(idiom, level) && isValidLevel(idiom, level) && isBasicExample(idiom)) {
    var parsed = mecab.parseSync(idiom);
    if (parsed.length != 1) {
      // 面倒なのでパス
    } else {
      var pos1 = parsed[0][1];
